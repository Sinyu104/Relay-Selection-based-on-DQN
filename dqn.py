    ...
    def _build_net(self):
        ...
        with tf.variable_scope('eval_net'):
            ...
            with tf.variable_scope('l1'):
                ...
                w1 = tf.get_variable('w1', [2, 2, self.n_features, n_l1],
                                     initializer=w_initializer,
                                     collections=c_names)
                b1 = tf.get_variable('b1', [1, n_l1], initializer=b_initializer,
                                     collections=c_names)
                self.s = tf.reshape(self.s, [-1, 1, 1, 2])
                conv = tf.nn.conv2d(self.s, w1, [1, 1, 1, 1], padding='SAME')

                l1 = tf.nn.relu(conv + b1)

            # 池化层
            p1 = tf.nn.max_pool(l1, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1],
                                padding='SAME')
            ...
            with tf.variable_scope('l2'):
                ...
                b2 = tf.get_variable('b2',
                                     ...
                                     initializer=b_initializer,
                                     collections=c_names)
                p_result = tf.reshape(p1, [-1, 1 * 1 * 10])
                self.q_eval = tf.matmul(p_result, w2) + b2
        ...
        with tf.variable_scope('target_net'):
            ...
            with tf.variable_scope('l2'):
                ...
                b2 = tf.get_variable('b2',
                                     ...
                                     collections=c_names)
                p_result = tf.reshape(p1, [-1, 1 * 1 * 10])
                self.q_next = tf.matmul(p_result, w2) + b2
